...
double* candidateSelection(struct candidateList **windowLists, long length){
  ...
  while (i<length){
    // at this point in the loop, there are no candidates before hand
    // By default all of the candidates in windowLists[start] have a
    // cost of zero and an indexLowestCost of zero

    // iterate through candidate lists until we encounter a list with at least
    // 1 element
    ...
    // stop when the last frame has been reached or when an empty list is
    // encountered
    ...
    // at this point we have either iterated through the candidate lists for 
    // all windows OR we have hit an empty candidate list
    if (start-i==1){
      
      // this is if window i has no candidates and it is preceeded by only 1
      // window with candidates. In other words, before the preceeding window
      // there are no windows or there is a window with zero candidates
      // alternatively, window i-1 was the last window and it is either the
      // first window or was preceeded by a window with zero candidates

      // for both cases we need to do the same thing for window i-1
      // for now we will just set the frequency to the first candidate
      fundamentals[i-1] = ((prevWindowList->array[0]).frequency);

      // for the scenario where window i has no candidates, the frequency
      // will be set to 0 on the next loop through 
    }
    else if (start-i > 1){
      // this is if window i has 0 candidates or window i-1 was the last
      // frame
...
void chooseLowestCost(double* fundamentals, struct candidateList **windowList,
  ...
  // this function finds the lowest cost path from windowList[final] back to
  // windowList[start] and fills in fundamentals with the frequency values

  // first, find the lowest cost candidate in windowList[final]
