...
void findpeaksHelper(double* x, double* y, long length, int peakgroup,
	...
	if (peakgroup<5){
		*peakY = -1.0;
		...
		// I should calculate width of the peak but that seems difficult and I 
...
double* quadFit(double* x, double* y, long length, double* mean, double *std){
	// Fits a Quadratic to data using least square fitting, like matlab's polyfit 
	// function
	// https://www.mathworks.com/help/matlab/ref/polyfit.html
	
	// Like polyfit, we center x at zero and scale it to have 1 unit standard deviation
	// xp = (x-mean)/std
	// Here mean is the average x value and std is the result of matlab's Std function 
	...
	*std = sqrt(*std/((double)(length-1)));
	...
	// S_y1 -> sxy
	// S_y2 -> sx2y
	double sxx, sxx2, sx2x2,sxy,sx2y, sumx, sumx2, sumy, cur_x, cur_x2,cur_y;
